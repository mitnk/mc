...
class CostManager(models.Manager):
    ...
    def get_last_month_net_cost(self, date_from=None):
        last_month = get_1st_of_last_month(date_from=date_from)
        cs = self.filter(added__month=last_month.month)
        return "%.2f" % sum([x.amount for x in cs if x.amount < BIG_AMOUNT])
    ...
    def get_this_month_net_cost(self, date_from=None):
        today = date_from or datetime.date.today()
        cs = self.filter(added__month=today.month)
        return "%.2f" % sum([x.amount for x in cs if x.amount < BIG_AMOUNT])
    ...
    def guess_this_month_total_cost(self, date_from=None):
        today = date_from or datetime.date.today()
        cs = self.filter(added__month=today.month)
        total = sum([x.amount for x in cs if x.amount < BIG_AMOUNT])
        bigs = self.filter(added__month=today.month, amount__gte=BIG_AMOUNT)
